...
tag = 'pbmc' # useful for saving figures
data_dir <- 'ccAF_improve/pbmc/filtered_gene_bc_matrices/hg19/' # where your data is located
...
#------ USE ENSEMBL IDs IF WANT TO SAVE OUT AS LOOM FILE TO APPLY CCAF --------#
#mitogenes <- read_csv("ccAF_newData/hBMMSC/mito_genes.csv")
...
abline(v = 1000, col = "red", lwd =3, lty =2)
abline(v = 8500, col = "red", lwd =3, lty =2)
abline(h = 0.009 , col = "red", lwd =3, lty =2)
abline (h = 0.07, col = "red", lwd =3, lty =2)
dev.off()
...
keep.detect <- which(seurat1@meta.data$percent.mito < 0.7 & seurat1@meta.data$percent.mito > 0.009 &
                        seurat1@meta.data$nCount_RNA < 8500 & seurat1@meta.data$nCount_RNA > 1000)
...
abline(v = 8500, col = "red", lwd =3, lty =2)
abline(h = 0.009 , col = "red", lwd =3, lty =2)
abline (h = 0.7, col = "red", lwd =3, lty =2)
dev.off()
...
#data_loom <- as.loom(seurat1, file.path(resdir, paste0(tag, "_data.loom")), verbose = FALSE)
#data_loom
#data_loom$close_all()
...
ccAF <- read_csv(file.path(resdir,paste0(tag, "_ccAF_calls.csv")))
...
#data_loom <- as.loom(seurat2, file.path(resdir, paste0(tag, "_data_norm.loom")), verbose = FALSE)
#data_loom
#data_loom$close_all()
...
m1 = DimPlot(seurat2, reduction = "umap") # if you use TNSE, replace "umap" with "tsne" in reduction parameter
dev.off()
...
# Visualize
m1 = DimPlot(seurat2, reduction = "umap", label.size = 6) + ggtitle ("de novo clustering")
af = DimPlot(seurat2, reduction = "umap", group.by="ccAF") + ggtitle("ccAF")
...
c1 =  DimPlot(seurat2, reduction = "umap", group.by="Phase") + ggtitle("ccSeurat")
...
#---------------------------------------------------
# Subsetting T cell clusters
#---------------------------------------------------

tag = "pbmc_tcells"
Cluster.0.1.9.only <- subset(x = seurat2, idents = c('0','1', '9'))
DefaultAssay(Cluster.0.1.9.only) <- "RNA"
Cluster.0.1.9.only <- SCTransform(Cluster.0.1.9.only, vars.to.regress = "nCount_RNA", verbose = FALSE)

Cluster.0.1.9.only <- RunPCA(Cluster.0.1.9.only, dims = 1:30)
Cluster.0.1.9.only <- RunUMAP(Cluster.0.1.9.only,   dims = 1:30)
Cluster.0.1.9.only <- FindNeighbors(Cluster.0.1.9.only,   dims = 1:30)
Cluster.0.1.9.only <- FindClusters(Cluster.0.1.9.only)

# Visualize
m1 = DimPlot(Cluster.0.1.9.only, reduction = "umap", label.size = 6) + ggtitle ("de novo clustering")
af = DimPlot(Cluster.0.1.9.only, reduction = "umap", group.by="ccAF") + ggtitle("ccAF")
c1 =  DimPlot(Cluster.0.1.9.only, reduction = "umap", group.by="Phase") + ggtitle("ccSeurat")
...
new.cluster.ids <- c("Naive CD4+ T", "celltype2", "celltype3") # number of new cluster IDs needs to be same as number of leiden clusters (also in same order - leiden cluster 0 = "celltype1" in this list and so on...)
