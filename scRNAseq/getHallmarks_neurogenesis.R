...
l1$Neurogenesis = c('GO:0022008')
l1$Developmental_progress = c('GO:0032502', 'GO:0044767')
...
for(file1 in files) {
  ...
  write.csv(hallmarks, file.path(opt$savedir, paste0(tag,"_jiangConrath_hallmarks_neurogenesis.csv")))
  tmp2 = subset(data.frame(hallmarks), rowSums(data.frame(hallmarks)[1:3] >= 0.8) > 0)
  if (!(dim(tmp2)[1[1]]==0)) {
      write.csv(tmp2, file.path(opt$savedirFilt, paste0(tag,"_jiangConrath_hallmarks_neurogenesis.csv")))
      }
}
