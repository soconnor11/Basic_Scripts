...
# Load up loom file
print('\nLoading data...')
ccAF1_scanpy = sc.read_loom('datasets/hBMMSC_data_POST_V2.loom')
#ccAF1_scanpy = sc.read_loom('datasets/cellcycle_int_integrated_V3_6_18_2020.loom')

# Load up conversion file
symEnsmbl = pd.read_csv('ccAF_improve/filtered_feature_bc_matrix/features.tsv.gz', header = None, index_col=1, sep='\t')
...
#tmp1 = set(list(pd.read_csv('markerGenes/hBMMSC_cluster_markers.csv', header = 0, index_col = 0).loc[:,'x']))
tmp1 = pd.read_csv('markerGenes/hBMMSC_cluster_markers.csv', header = 0, index_col = 0)
#len(tmp1) #2689
tmp1 = set(list(tmp1['gene']))
tmp2 = []
missed = []
for j in tmp1:
...
mg1 = tmp2 # Only 2672, only garbage genes are lost that cannot be mapped anymore
...
# Make folder for downstream results
...
for k in range(nfolds):
    ...
    truelab.extend(ccAF1_scanpy.obs['new_clusters'][samp1])
    trainInds.append(np.setdiff1d(allInds, samp1))
    mapMe_SVMrejRF.append([k, ccAF1_scanpy[trainInds[k],:], ccAF1_scanpy.obs['new_clusters'][trainInds[k]], ccAF1_scanpy[testInds[k],:]])
...
if not exists('results/SVMrej/CV_classification_report_'+str(ccAF1_scanpy_mg1._n_vars)+'_V2.csv'):
    ...
    states1 = list(ccAF1_scanpy_mg1.obs['new_clusters'].unique())
...
if not exists('results/RFpy/CV_classification_report_'+str(ccAF1_scanpy_mg1._n_vars)+'_V2.csv'):
...
if not exists('results/KNN/CV_classification_report_'+str(ccAF1_scanpy_mg1._n_vars)+'.csv'):
    ...
    for k in range(nfolds):
        ...
        KNN = cl.Classifier_KNN(ccAF1_scanpy_mg1[trainInds[k],:], 'new_clusters')
...
if not exists('results/ACTINN/CV_classification_report_'+str(ccAF1_scanpy_mg1._n_vars)+'.csv'):
    ...
    for k in range(nfolds):
        ...
        ACTINN = cl.Classifier_ACTINN(train = ccAF1_scanpy_mg1[trainInds[k],:], label = 'new_clusters')
...
if not exists('results/ACTINN/ccAF_'+str(ccAF1_scanpy_mg1._n_vars)+'.pkl'):
    ACTINN = cl.Classifier_ACTINN(train = ccAF1_scanpy_mg1, label = 'new_clusters')
...
nsc = sc.read_loom('cellcycle_int_integrated_V3_6_18_2020.loom')
...
nsc.var_names = pd.Index(symEnsmbl.loc[nsc.var_names,0], name='Ensembl')
